...
class FlowRender
{
    ...
    /**
     * Textos das NOTAS DE FIM (nós `endnote`), na ordem do documento — para a lista
     * no fim. Espelha core/endnotes.collectEndnotes.
     *
     * @param  array<int,array<string,mixed>>  $sections
     * @return list<string>
     */
    public static function collectEndnotes(array $sections): array
    {
        $out = [];
        $walkInline = function ($node) use (&$walkInline, &$out): void {
            if (! is_array($node)) {
                return;
            }
        ...
        $walkFlow = function (array $flow) use (&$walkFlow, &$walkInline): void {
            foreach ($flow as $n) {
                if (! is_array($n)) {
                    continue;
                }
                ...
                if ($type === 'columnSet') {
                    ...
                    $walkFlow($n['flow'] ?? []);
                } elseif ($type === 'text' && is_array($n['content'] ?? null)) {
