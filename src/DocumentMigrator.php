...
class DocumentMigrator
{
    ...
    private static function migrateStructure(array $st): array
    {
        if (($st['variant'] ?? null) === 'banner') {
            ...
            if (isset($st['dataBinding'])) $group['dataBinding'] = $st['dataBinding'];
            return [$group];
        }

        $cols = $st['columns'] ?? [];
        if (
            count($cols) === 1 && abs(($cols[0]['width'] ?? 100) - 100) < self::WIDTH_EPS
            && self::isDefaultStyles($cols[0]['styles'] ?? null) && self::isDefaultStyles($st['styles'] ?? null)
            && self::isTrivialMeta($st['meta'] ?? null) && !isset($st['dataBinding'])
        ) {
        ...
        if (isset($st['dataBinding'])) $columnSet['dataBinding'] = $st['dataBinding'];
        return [$columnSet];
    }
    ...
    private static function migrateStripe(array $stripe): array
    {
        ...
        if (isset($stripe['dataBinding'])) $group['dataBinding'] = $stripe['dataBinding'];
        return [$group];
    }
    ...
    public static function v2ToV3(array $v2): array
    {
        ...
        foreach (['theme', 'emailSettings', 'createdAt', 'updatedAt'] as $k) if (isset($v2[$k])) $v3[$k] = $v2[$k];
        return $v3;
    }
    ...
    private static function columnSetToStructure(array $cs, callable $gen): array
    {
        ...
        if (isset($cs['dataBinding'])) $st['dataBinding'] = $cs['dataBinding'];
        return $st;
    }
    ...
    private static function groupToStructure(array $g, callable $gen): array
    {
        ...
        if (isset($g['dataBinding'])) $st['dataBinding'] = $g['dataBinding'];
        if (($g['variant'] ?? null) === 'banner') {
            $st['variant'] = 'banner';
            foreach (['backgroundImage', 'backgroundSize', 'backgroundPosition', 'minHeight', 'overlayColor', 'overlayOpacity'] as $k) {
    ...
    private static function groupToStripe(array $g, callable $gen): array
    {
        ...
        return ['id' => $g['id'] ?? $gen(), 'type' => 'stripe', 'meta' => $g['meta'] ?? self::defaultMeta(), 'styles' => $g['styles'] ?? self::defaultStyles(), 'contentMaxWidth' => $g['contentMaxWidth'] ?? 0, 'contentAlignment' => $g['contentAlignment'] ?? 'center', 'children' => self::flowToStructures($g['flow'] ?? [], $gen)];
    }
    ...
    public static function v3ToV2(array $v3): array
    {
        ...
        foreach (['theme', 'emailSettings', 'createdAt', 'updatedAt'] as $k) if (isset($v3[$k])) $v2[$k] = $v3[$k];
        return $v2;
    }
    ...
    public static function v3Envelope(array $v3): array
    {
        ...
        foreach (['theme', 'emailSettings', 'createdAt', 'updatedAt'] as $k) if (isset($v3[$k])) $env[$k] = $v3[$k];
        return $env;
    }
